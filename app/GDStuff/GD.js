...
class getLevel{
    levels(str, diff, len, page, total, uncompleted, completed, featured, original, twoPlayer, coins, epic, star) {
        if (!str) str = '';
        if (!diff) diff = '-';
        if (!len) len = '-';
        if (!page) page = '0';
            ...
            request.post({
                ...
                for (let i in levels) {
                    if (levels[i] == '-1') {
                        ...
                        // time to do some wacky stufffffffffffffffffffffffffffffffffff
